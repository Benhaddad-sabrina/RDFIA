# -*- coding: utf-8 -*-
"""BENHADDAD_BENSIDHOUM_2-d_GANs

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Mw-kXnRBezgWL04zEFdjHVM4b6p3Kpi_
...
def get_upscaling_block(channels_in, channels_out, kernel, stride, padding, last_layer=False):
    ...
    - last_layer (bool): If True, uses Tanh activation for the last layer. Defaults to False.
    '''

    layers = [nn.ConvTranspose2d(channels_in, channels_out, kernel, stride=stride, padding=padding)]

    if not last_layer:
        layers.extend([nn.BatchNorm2d(channels_out), nn.ReLU(inplace = True)])
    else:
...
def get_downscaling_block(channels_in, channels_out, kernel, stride, padding, use_batch_norm=True, is_last=False):
    '''
    Creates a downscaling block with a convolution layer, optionally followed by batch normalization and an activation function.
    The last layer can be specified to use a different activation function, defaulting to Sigmoid.
    ...
    - is_last (bool): If True, uses Sigmoid activation for the last layer. Defaults to False.
    '''

    layers = [nn.Conv2d(channels_in, channels_out, kernel, stride=stride, padding=padding)]

    if use_batch_norm and not is_last:
        layers.append(nn.BatchNorm2d(channels_out))

    activation = nn.Sigmoid() if is_last else nn. LeakyReLU(negative_slope=0.2, inplace =True)
...
def training(nb_epochs=5):
  ...
  for epoch in range(nb_epochs):
      ...
      for i, batch in pbar:
          ...
          yhat_fake = netD(fake_images.detach())
          ...
          real_image_labels = get_labels_one(cur_batch_size)
          fake_image_labels = get_labels_zero(cur_batch_size)

          d_loss_fake_images = criterion(yhat_fake, fake_image_labels)
          d_loss_real_images = criterion(yhat_real, real_image_labels)


          d_loss = d_loss_fake_images + d_loss_real_images
          d_opt.zero_grad()
          ...
          # we compute a new yhat after updating the discriminator
          # pas obligatoire a skip

          # Sample a new z vector
          new_z = sample_z(cur_batch_size, nz)

          # Generate new fake images
          new_fake_images = netG(z)

          # Classify new fake images with D
          yhat_new_fake = netD(new_fake_images)

          g_loss = criterion(yhat_new_fake, real_image_labels) # Use real labels as target for fake images / same as before
          g_opt.zero_grad()
...
"""## Analyzing GAN Performance: Functionality and Limitations of Generative Architectures

### Generate the images corresponding to several linear interpolations
"""
...
for alpha in alpha_values:
  z = alpha * z1 + (1-alpha) * z2 # compute the linear interpolation
  img = netG(z)
  un_norm = renorm(img)
  ...
  print(f"generated images with alpha = {alpha}")
  plt.imshow(pil_grid)
  plt.show()

"""### Increase *ndf*

We intend to significantly decrease the value of ndf to deliberately weaken the discriminator, thereby creating an opportunity for the generator to more easily fool the discriminator in our Generative Adversarial Network.
"""
...
"""### Replace the custom weight initialization with Pytorch's default initialisation"""
...
"""### Increase sightly the learning rate of the generator"""
...
training(nb_epochs=30)

"""### Increase significantly nz"""
